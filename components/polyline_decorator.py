...
polygon = dl.Polygon(
    color="#ff7800",
    weight=1,
    positions=[[[54, -6], [55, -7], [56, -2], [55, 1], [53, 0]], [[54, -3], [54, -2], [55, -1], [55, -5]]],
)
...
patterns = [
    dict(offset="12", repeat="25", dash=dict(pixelSize=10, pathOptions=dict(color="#f00", weight=2))),
...
patterns = [
    dict(repeat="10", dash=dict(pixelSize=5, pathOptions=dict(color="#000", weight=1, opacity=0.2))),
    dict(offset="16%", repeat="33%", marker=marker),
]
rotated_markers = dl.PolylineDecorator(
    positions=[[42.9, -15], [44.18, -11.4], [45.77, -8.0], [47.61, -6.4], [49.41, -6.1], [51.01, -7.2]],
    patterns=patterns,
)
# Create app.
app = DashProxy()
